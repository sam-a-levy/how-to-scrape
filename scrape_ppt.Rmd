...
1. **Difference between APIs & scraping**
...
4. Select the HTML and parse it in the appropriate data type (shapefile, datatable etc.)

## Working with HTML
- HTML uses a tree structure of nodes (also called elements), styled using CSS
- A node will have HTML tags & can have CSS classes/other features `<tag.class>`
...
html <- read_html(url3) # read html
node <- html_element(html,".table-striped.agk-cont-tb1") # extract node
dat_scrape <- html_table(node,dec=",") # convert to data table
```
```{r message=FALSE, warning=FALSE, echo=FALSE}
...
- Much of the data online you may want to scrape will not be already structured into tables
...
- identifying correct node & limitations of this with messy data

## Things not covered
- how to deal with scrapes that don't work (& thus break your code)
- dynamic websites + how to deal with (selenium / POST)

## Conclusion
- A lot of data online is available through APIs, even when it doesn't appear to be
- Where possible, use an API, as it will almost always be easier & more reliable than scraping
